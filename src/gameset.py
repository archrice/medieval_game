...
def init():
    ...
    file1 = f'{cwd}/charactername.json'

    with open(file1, 'w') as db:
        json.dump(cname, db)

    age = input("What will your character's age be?\n::")

    with open(f'{cwd}/age.json','w') as f:
        json.dump(age,f)

    with open(f'{cwd}/cl.json','w') as f:
        cl=0
        json.dump(cl,f)
...
def load_game():
    with open(f'{cwd}/level.json') as f:
        level = f.read()
        f.close()
    with open(f"{cwd}/age.json") as f:
        age = f.read()
        f.close()
    with open(f'{cwd}/reputation.json') as f:
        reputation = f.read()
        f.close()
    with open(f'{cwd}/charactername.json') as f:
        cname = f.read()
        f.close()
    with open(f"{cwd}/reputation.json") as f:
        reputation = f.read()
...
def save(level, age, cname, reputation):
    with open(f'{cwd}/level.json','w') as f:
        json.dump(level, f)
    with open(f'{cwd}/age.json','w') as f:
        f.write(age)
        f.close()
    with open(f'{cwd}/charactername.json','w') as f:
        f.write(cname)
        f.close()
    with open(f'{cwd}/reputation.json','w') as f:
